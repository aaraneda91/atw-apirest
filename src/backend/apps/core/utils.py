                ...
                data = [
                    {
                        ...
                        'articulo_cantidad': articulo.cantidad,
...
def eliminar_articulo_carrito(usuario_id, articulo_id):
    if usuario_id is not None and articulo_id:
        carrito = Carrito.objects.filter(usuario = usuario_id).first() #primero buscamos el carrito del usuario
        ArticulosCarrito.objects.filter(carrito = carrito, articulo = articulo_id).delete() #buscamos el articulo del carrito identificado
        return True
    else:
        raise Http404